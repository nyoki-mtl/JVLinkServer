...
void parseO5Fields(ParsedRecord& record) {
  ...
  // 項番13: 発売フラグ 三連複 (位置:40, バイト:1)
  record.structured_data["trio_sales_flag"] = record.extractAndConvert(39, 1);
  // 項番14: 三連複オッズ (位置:41, 12240バイト(15バイト×816組))
  nlohmann::json trio_odds_array = nlohmann::json::array();
  for (int i = 0; i < 816; i++) {
    size_t base_pos = 40 + (i * 15);
    nlohmann::json odds_entry;
    // 項番14a: 組番 (ブロック内位置1, 6バイト)
    odds_entry["combination"] = record.extractAndConvert(base_pos, 6);
    // 項番14b: オッズ (ブロック内位置7, 6バイト)
    odds_entry["odds"] = record.extractAndConvert(base_pos + 6, 6);
    // 項番14c: 人気順 (ブロック内位置13, 3バイト)
    odds_entry["popularity_rank"] = record.extractAndConvert(base_pos + 12, 3);
    trio_odds_array.push_back(odds_entry);
  }
  record.structured_data["trio_odds"] = trio_odds_array;
  // 項番15: 三連複票数合計 (位置:12281, バイト:11)
  record.structured_data["trio_total_votes"] = record.extractAndConvert(12280, 11);
}
