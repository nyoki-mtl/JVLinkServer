...
def test_inject_bundled_executable_into_wheel_updates_record(tmp_path: Path) -> None:
    wheel_path = tmp_path / "pyjvlink-0.8.2-py3-none-win_amd64.whl"
    bundled_exe_path = tmp_path / "JVLinkServer.exe"
    bundled_exe_path.write_bytes(b"fake-exe")

    with zipfile.ZipFile(wheel_path, "w", compression=zipfile.ZIP_DEFLATED) as wheel:
        wheel.writestr("pyjvlink/__init__.py", "__version__ = '0.8.2'\n")
        wheel.writestr(
            "pyjvlink-0.8.2.dist-info/RECORD",
    ...
    with zipfile.ZipFile(wheel_path) as wheel:
        ...
        rows = list(csv.reader(wheel.read("pyjvlink-0.8.2.dist-info/RECORD").decode("utf-8").splitlines()))
